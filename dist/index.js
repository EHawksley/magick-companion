...
var dreamEntries = mysqlTable("dream_entries", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
...
// server/magick.ts
async function getJournalAIFeedback(entry, recentEntries, entryDate) {
  ...
  const patternNote = topPlanet ? `Pattern note: this practitioner has logged ${recentEntries.length} entries total. Most common planetary hour during practice: ${topPlanet[0]} (${topPlanet[1]} times).` : "";
...
async function getDreamAIFeedback(entry, recentDreams, recentRituals, entryDate) {
  const clarityLabels = { 1: "Vague", 2: "Clear", 3: "Vivid / Lucid" };
  const ritualStr = recentRituals.slice(0, 5).map(
    (r) => `[${new Date(r.createdAt).toLocaleDateString()}] ${r.ritualType}${r.planetaryHour ? ` (${r.planetaryHour} hour)` : ""}: ${r.notes.slice(0, 150)}`
  ).join("\n");
  const dreamStr = recentDreams.slice(0, 5).map(
    (d) => `[${new Date(d.createdAt).toLocaleDateString()}] ${d.moonPhase ?? ""} \xB7 ${d.dayRuler ?? ""}'s day: ${d.dream.slice(0, 150)}`
  ).join("\n");
...
${entry.hoursAfterRitual !== null && entry.hoursAfterRitual !== void 0 ? `Hours since last ritual: ${entry.hoursAfterRitual}h` : ""}
...
var journalRouter = router({
  list: protectedProcedure.query(async ({ ctx }) => {
    const db = await getDb();
    if (!db) return [];
    return db.select().from(journalEntries).where(eq2(journalEntries.userId, ctx.user.id)).orderBy(desc(journalEntries.createdAt)).limit(100);
  }),
  create: protectedProcedure.input(z2.object({
    ritualType: z2.string().min(1),
    notes: z2.string().min(1),
    planetaryHour: z2.string().optional(),
    hourNumber: z2.number().optional(),
    isDay: z2.boolean().optional(),
    moonPhase: z2.string().optional(),
    dayRuler: z2.string().optional(),
    isPlanetDay: z2.boolean().optional()
  })).mutation(async ({ ctx, input }) => {
    const db = await getDb();
    if (!db) throw new Error("Database unavailable");
    const recent = await db.select().from(journalEntries).where(eq2(journalEntries.userId, ctx.user.id)).orderBy(desc(journalEntries.createdAt)).limit(10);
    const now = /* @__PURE__ */ new Date();
    let aiFeedback = null;
    try {
    ...
    const [result] = await db.insert(journalEntries).values({
      ...
      isDay: input.isDay !== void 0 ? input.isDay ? 1 : 0 : null,
      moonPhase: input.moonPhase ?? null,
      dayRuler: input.dayRuler ?? null,
      isPlanetDay: input.isPlanetDay !== void 0 ? input.isPlanetDay ? 1 : 0 : null
    });
    void result;
    const [created] = await db.select().from(journalEntries).where(eq2(journalEntries.userId, ctx.user.id)).orderBy(desc(journalEntries.createdAt)).limit(1);
    return created;
  }),
  update: protectedProcedure.input(z2.object({ id: z2.number(), notes: z2.string().min(1) })).mutation(async ({ ctx, input }) => {
    const db = await getDb();
    if (!db) throw new Error("Database unavailable");
    await db.update(journalEntries).set({ notes: input.notes }).where(eq2(journalEntries.id, input.id));
    return { success: true };
  }),
  stats: protectedProcedure.query(async ({ ctx }) => {
    const db = await getDb();
    if (!db) return { streak: 0, total: 0, lastFocus: null };
    const entries = await db.select().from(journalEntries).where(eq2(journalEntries.userId, ctx.user.id)).orderBy(desc(journalEntries.createdAt)).limit(200);
...
var dreamsRouter = router({
  list: protectedProcedure.query(async ({ ctx }) => {
    const db = await getDb();
    if (!db) return [];
    return db.select().from(dreamEntries).where(eq2(dreamEntries.userId, ctx.user.id)).orderBy(desc(dreamEntries.createdAt)).limit(100);
  }),
  create: protectedProcedure.input(z2.object({
    dream: z2.string().min(1),
    symbols: z2.string().optional(),
    emotion: z2.string().optional(),
    clarity: z2.number().int().min(1).max(3),
    moonPhase: z2.string().optional(),
    dayRuler: z2.string().optional(),
    hoursAfterRitual: z2.number().optional()
  })).mutation(async ({ ctx, input }) => {
    const db = await getDb();
    if (!db) throw new Error("Database unavailable");
    const recentDreams = await db.select().from(dreamEntries).where(eq2(dreamEntries.userId, ctx.user.id)).orderBy(desc(dreamEntries.createdAt)).limit(5);
    const recentRituals = await db.select().from(journalEntries).where(eq2(journalEntries.userId, ctx.user.id)).orderBy(desc(journalEntries.createdAt)).limit(5);
    const now = /* @__PURE__ */ new Date();
    let aiFeedback = null;
    try {
    ...
    const [created] = await db.select().from(dreamEntries).where(eq2(dreamEntries.userId, ctx.user.id)).orderBy(desc(dreamEntries.createdAt)).limit(1);
    return created;
  })
});
